...
parser.add_argument('--input_dir', type=str, default='cache')
...
parser.add_argument('--chop_forward', type=bool, default=True, help='Reduces image memory allocation for low-memory environments')
...
def eval(opt):
    ...
    if cuda and not torch.cuda.is_available():
        raise Exception("No GPU found, set --gpu_mode False")
    ...
    for batch in testing_data_loader:
        ...
        del batch, input, bicubic, prediction
        gc.collect()
...
def save_img(img, img_name):
    ...
    save_img = cv2.cvtColor(save_img*255, cv2.COLOR_BGR2RGB)
    ...
    cv2.imwrite(save_fn, save_img,  [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
def simple_enhance(img_path):
    image = Image.open(img_path)
    #Brightness
    im1 = ImageEnhance.Brightness(image)
    im1 = im1.enhance(1.20)
    #Contrast
    im2 = ImageEnhance.Contrast(im1)
    im2 = im2.enhance(1.20)
    #Sharpness
...
for image in os.listdir(input_dir):
    img = simple_enhance(input_dir+image)
    img = check_size(img)
