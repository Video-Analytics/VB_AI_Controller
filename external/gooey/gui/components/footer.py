...
class Footer(wx.Panel):
    ...
    def updateTimeRemaining(self,*args,**kwargs):
        ...
        if elapsed_time_value is None:
            ...
            self.time_remaining_text.SetLabel("Time Elapsed: "+f"{elapsed_time_value}"+"-->"+"Estimated Time Remaining: "+f"{estimate_time_remaining}")
            return
        else:
            self.time_remaining_text.SetLabel("Time Elapsed: "+f"{elapsed_time_value}")
